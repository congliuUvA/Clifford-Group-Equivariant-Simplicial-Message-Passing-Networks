...
for molecule_type in molecule_list:
        ...
        for j in range(n):
            if i != j:
                ...
                if _d < _lambda:
                    atom_edges[i][j] = 1
    # print(atom_edges)
    np.save(dataroot + '/' + molecule_type + '_charges.npy', z)
    np.save(dataroot + '/'+ molecule_type + '_structure.npy', atom_edges)
    ...
    np.save(dataroot + '/'+ molecule_type + '_train.npy', train_data)
    np.save(dataroot + '/'+ molecule_type + '_val.npy', val_data)
    np.save(dataroot + '/'+ molecule_type + '_test.npy', test_data)